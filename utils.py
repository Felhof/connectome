    ...
    for target, source in tqdm(important_connections):
        ...
        for layers in tqdm(layer_powerset, leave=False, desc=f"{source} -> {target}"):
            ...
            if abs(result) > threshold:
                print(f"{source}, {str_tokens[source]} -> {target}, {str_tokens[target]} {layers} {result}")
...
def ioi_metric(s_token_idx: int, io_token_idx: int) -> Callable[..., float]:
    def metric(original_logits: Float[Tensor, 'seq vocab'],
        ...
        baseline = original_logits[-1, io_token_idx] - original_logits[-1, s_token_idx]
        logit_diff = patched_logits[-1, io_token_idx] - patched_logits[-1, s_token_idx]
        return (logit_diff - baseline).item()
    return metric

def docstring_metric(correct_param_id: int, incorrect_param_ids: List[int]) -> Callable[..., float]:
    def metric(original_logits: Float[Tensor, 'seq vocab'],
               patched_logits: Float[Tensor, 'seq vocab'],
              ) -> float:
        original_incorrect_logit = max(
           original_logits[-1, incorrect_param_id].item() for incorrect_param_id in incorrect_param_ids 
        )
        baseline = original_logits[-1, correct_param_id] - original_incorrect_logit
        patched_incorrect_logit = max(
           patched_logits[-1, incorrect_param_id].item() for incorrect_param_id in incorrect_param_ids 
        )
        logit_diff = patched_logits[-1, correct_param_id] - patched_incorrect_logit
        return (logit_diff - baseline).item()
    return metric
...
def sankey_diagram_of_connectome(
        model: HookedTransformer, 
        prompt: str, 
        connectome: List[Tuple[int, int, List[int], float]]
    ):
    node_labels = [f"{idx + 1}, '{token}'" for idx, token in enumerate(model.to_str_tokens(prompt)[1:])]
    ...
    max_connection_strength = max(abs(score) for _, _, _, score in connectome)

    for source, target, layers, score in connectome:
        sources.append(source - 1)
        targets.append(target - 1)
        oppacity = abs(score) / max_connection_strength * 0.8

        color = f'rgba(255,0,0, {oppacity})' if score > 0 else f'rgba(0,0,255, {oppacity})'
        link_colors.append(color)
        values.append(abs(score))
        link_labels.append(str(layers))
