...
def corrupt_sentences(model, tokenizer, sentences, n_masks=4, last_mask_idx=-1):

    model.eval()
    masked_sentences = []

    token_ids = tokenizer(sentences, return_tensors="pt", padding=True)["input_ids"]
    ...
    mask_indices = torch.sort(torch.randperm(last_mask_idx + 1)[:n_masks]).values.unsqueeze(0).expand(n_batch, -1) + 1
...
# %%
original_logit_diff = (original_logits[0,-1,gpt2_small.to_single_token(" Alice")] - original_logits[0,-1,gpt2_small.to_single_token(" Bob")]).item()
# %%
# 
# %%
...
for layer in range(gpt2_small.cfg.n_layers):
    cache[utils.get_act_name("v", layer)][:,source_pos] = corrupted_cache[utils.get_act_name("v", layer)][:,source_pos]
    cache[utils.get_act_name("k", layer)][:,source_pos] = corrupted_cache[utils.get_act_name("k", layer)][:,source_pos]
# %%
hooks = [
    (
        utils.get_act_name("v", layer),
        partial(patch_position, cache=cache[utils.get_act_name("v", layer)], source=source_pos, target=target_pos)
    ) for layer in range(gpt2_small.cfg.n_layers)
] + [
    (
        utils.get_act_name("k", layer),
        partial(patch_position, cache=cache[utils.get_act_name("k", layer)], source=source_pos, target=target_pos)
    ) for layer in range(gpt2_small.cfg.n_layers)
]
corrupted_logits = gpt2_small.run_with_hooks(
    original_prompt,
    fwd_hooks=hooks
)
# %%
corrupted_logit_diff = (corrupted_logits[0,-1,gpt2_small.to_single_token(" Alice")] - corrupted_logits[0,-1,gpt2_small.to_single_token(" Bob")]).item()
# %%
