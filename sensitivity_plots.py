...
def get_similar_word(model, word, distance=1):
    ...
    similarities = (model.W_E[model.to_single_token(word)] @ model.W_E.T).clone()
...
gc.collect()
# %%
def get_similar_word_softmax(model, word, temperature=1., k=20):
    similarities = (model.W_E[model.to_single_token(word)] @ model.W_E.T).clone()
    top_values, top_indices = torch.topk(similarities, k=k)
    probabilites = (top_values / temperature).softmax(0)
    replacement_id = top_indices[torch.distributions.Categorical(probabilites).sample()]
    return model.to_single_str_token(replacement_id.item())

# %%
def corrupt_prompt_with_similar_words(model, original_str_tokens, replace_pos_up_to, n_replacements, distance):
    str_tokens = original_str_tokens.copy()
    ids_to_replace = np.random.choice(range(1, replace_pos_up_to), n_replacements, replace=False)
    for id in ids_to_replace:
        str_tokens[id] = get_similar_word(model, str_tokens[id], distance=distance)
        torch.cuda.empty_cache()
        gc.collect()
    corrupted_prompt = model.tokenizer.batch_decode([[model.to_single_token(t) for t in str_tokens][1:]])
    return corrupted_prompt
# %%
def corrupt_prompt_with_softmax(model, original_str_tokens, replace_pos_up_to, temperature):
    str_tokens = original_str_tokens.copy()
...
for distance in distances:
    ...
    for _ in range(n_samples):
        corrupted_prompt = corrupt_prompt_with_similar_words(model, original_str_tokens, replace_pos_up_to, n_replacements, distance)
...
for pos in source_pos:
    ...
    for idx, distance in enumerate(distances):
        pos_results_for_distance = [abs(r) for r in results[pos][idx*n_samples:(idx+1)*n_samples]]
